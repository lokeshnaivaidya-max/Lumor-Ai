  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="glass-input flex w-full items-center justify-between text-left min-w-0 gap-2"
      >
        <span className={`truncate ${value ? "" : "text-muted-foreground/60"}`}>{value ? selectedLabel : placeholder}</span>
...
function resizeImageToDataUrl(file: File, maxDim = 200, quality = 0.85): Promise<string> {
...
function AvatarUpload({ image, onChange }: { image: string; onChange: (v: string) => void }) {
  ...
  async function handleFile(file?: File | null) {
    ...
    try {
      setProgress(30)
      const compressedDataUrl = await resizeImageToDataUrl(file, 200, 0.85)
      setProgress(100)
...
export function ProfileClient({ user }: {
  ...
  async function saveProfile() {
    ...
    try {
      ...
      await authClient.updateUser({ name: name || undefined, image: image || null }).catch((err) => {
        console.warn("[Profile] authClient.updateUser warning:", err)
      })
      ...
      await logActivity({ type: "profile", title: "Updated profile information", href: "/profile" }).catch(() => {})
    } catch (e: any) {
      console.error("[Profile] saveProfile error:", e)
      const msg = e?.message || "Failed to save profile changes"
      setError(msg)
      toast(msg, "error")
    } finally {
      setBusy(false)
