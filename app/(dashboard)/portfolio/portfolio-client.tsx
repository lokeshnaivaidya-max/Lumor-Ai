...
import { Plus, TrendingUp, TrendingDown, PieChart, Activity, Brain, RefreshCw } from "lucide-react"
...
const SECTOR_DATA = [
  { name: "Technology", allocation: 70, color: "oklch(0.7 0.13 250)" },
  { name: "Automotive", allocation: 18, color: "oklch(0.66 0.1 268)" },
  { name: "Consumer Cyclical", allocation: 12, color: "oklch(0.8 0.13 168)" },
]
...
export function PortfolioClient() {
  ...
  const totalCost = MOCK_HOLDINGS.reduce((acc, h) => acc + h.shares * h.avgPrice, 0)
  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-3">
        <div className="glass-panel edge-light rounded-2xl p-5">
          <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Total Value</p>
          <p className="mt-1.5 text-2xl font-semibold tracking-tight text-emerald">{formatCurrency(totalValue)}</p>
        </div>
        <div className="glass-panel edge-light rounded-2xl p-5">
          <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Total P&L</p>
          <p className={`mt-1.5 text-2xl font-semibold tracking-tight ${totalPnL >= 0 ? "text-emerald" : "text-neg"}`}>
            {totalPnL >= 0 ? "+" : ""}{formatCurrency(totalPnL)}
            <span className="ml-1 text-sm">({pnlPercent >= 0 ? "+" : ""}{pnlPercent.toFixed(2)}%)</span>
          </p>
        </div>
        <div className="glass-panel edge-light rounded-2xl p-5">
          ...
          <div className="mt-2 flex items-center gap-3">
            ...
            <span className="text-lg font-semibold text-emerald">82</span>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6 flex gap-2">
        {(["holdings", "allocation", "performance"] as const).map((tab) => (
          <button
            ...
            className={`rounded-full px-4 py-2 text-sm capitalize transition-colors ${
              view === tab ? "bg-primary/10 text-primary font-medium" : "text-muted-foreground hover:text-foreground"
            }`}
          >
            {tab}
          </button>
        ...
        <div className="ml-auto flex gap-2">
          ...
          <button className="glass-strong flex items-center gap-1.5 rounded-full bg-primary/10 px-3 py-2 text-xs font-medium text-primary transition-colors hover:bg-primary/20">
      ...
      {view === "holdings" && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-panel edge-light rounded-2xl overflow-hidden">
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-border/30">
                {MOCK_HOLDINGS.map((h) => {
                  ...
                  return (
                    <tr key={h.symbol} className="transition-colors hover:bg-white/5">
                      <td className="px-5 py-4">
                        <span className="font-medium">{h.symbol}</span>
                      </td>
                      <td className="px-5 py-4 text-sm text-muted-foreground">{h.name}</td>
                      <td className="px-5 py-4 text-right text-sm">{h.shares}</td>
                      <td className="px-5 py-4 text-right text-sm">{formatCurrency(h.avgPrice)}</td>
                      <td className="px-5 py-4 text-right text-sm font-medium">{formatCurrency(h.currentPrice)}</td>
                      <td className={`px-5 py-4 text-right text-sm font-medium ${pnl >= 0 ? "text-emerald" : "text-neg"}`}>
                        {pnl >= 0 ? "+" : ""}{formatCurrency(pnl)} ({(pnlPct >= 0 ? "+" : "")}{pnlPct.toFixed(1)}%)
                      </td>
                      <td className="px-5 py-4 text-right text-sm">{h.allocation}%</td>
                      <td className="px-5 py-4 text-right">
                        <button className="rounded-lg border border-border px-2 py-1 text-xs text-muted-foreground transition-colors hover:text-foreground">Edit</button>
                      </td>
                    </tr>
                  )
                })}
      ...
      {view === "allocation" && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid gap-6 lg:grid-cols-2">
          <div className="glass-panel edge-light rounded-2xl p-6">
            <h3 className="mb-4 text-sm font-medium">Sector Distribution</h3>
          ...
          <div className="glass-panel edge-light rounded-2xl p-6">
            <h3 className="mb-4 text-sm font-medium">Holdings Allocation</h3>
              ...
              <div className="relative h-40 w-40">
                <svg viewBox="0 0 100 100" className="h-full w-full -rotate-90">
                  {MOCK_HOLDINGS.map((h, i) => {
                    ...
                    const colors = ["oklch(0.7 0.13 250)", "oklch(0.66 0.1 268)", "oklch(0.8 0.13 168)", "oklch(0.87 0.08 88)", "oklch(0.68 0.17 250)"]
                    return (
                      <circle key={h.symbol} cx="50" cy="50" r="38" fill="none" stroke={colors[i % colors.length]} strokeWidth="8"
      ...
      {view === "performance" && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-panel edge-light rounded-2xl p-6">
          <h3 className="mb-4 text-sm font-medium">Performance Analytics</h3>
