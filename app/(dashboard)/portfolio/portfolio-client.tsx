...
function fmt(n: number, sym?: string) {
  return formatCurrency(n, sym ? { symbol: sym } : "USD")
}
                      ...
                      {holdings.map((h, idx) => {
                        ...
                        return (
                          <motion.tr initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.05, duration: 0.5 }} className="group transition-colors duration-200 hover:bg-[var(--panel-2)]/60" style={{ borderColor: "var(--line)" }}>
                            ...
                            <td className="px-6 py-4 text-right font-mono text-sm tabular-nums text-[var(--text-primary)]" style={{ borderBottom: "1px solid var(--line)" }}>{fmt(h.avgPrice, h.symbol)}</td>
                            <td className="px-6 py-4 text-right font-mono text-sm font-semibold tabular-nums text-[var(--text-primary)]" style={{ borderBottom: "1px solid var(--line)" }}>{fmt(h.price, h.symbol)}</td>
