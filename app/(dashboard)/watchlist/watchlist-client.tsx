        ...
        <div className="overflow-x-auto">
          <table className="w-full">
                ...
                {(["symbol", "name", "price", "change", "rating"] as const).map((k) => (
                  <th key={k} className="cursor-pointer px-5 py-4 text-left" onClick={() => { setSortKey(k); setSortAsc((a) => !a) }}>
                    <div className="flex items-center gap-1">
                      {k} <ChevronDown className={`h-3 w-3 transition-transform ${sortKey === k ? sortAsc ? "rotate-180" : "" : "opacity-30"}`} />
                    </div>
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-border/30">
              {filtered.map((w) => (
                <tr key={w.symbol} className="transition-colors hover:bg-white/[0.02]">
                  <td className="px-5 py-4"><span className="font-medium">{w.symbol}</span></td>
                  <td className="px-5 py-4 text-sm text-muted-foreground">{w.name}</td>
                  <td className="px-5 py-4 text-sm font-medium tabular-nums">{fmt(w.price)}</td>
                  <td className={`px-5 py-4 text-sm font-medium tabular-nums ${w.change >= 0 ? "text-emerald" : "text-neg"}`}>
                    <div className="flex items-center gap-1">
