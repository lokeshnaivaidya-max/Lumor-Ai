    ...
    >
      <div className="flex items-start justify-between p-5">
        <div className="flex items-center gap-3">
          {item.logo ? (
            <img src={item.logo} alt="" width={36} height={36} className="h-9 w-9 rounded-xl bg-card object-contain" onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = "none" }} />
          ) : (
            <div className={`flex h-9 w-9 items-center justify-center rounded-2xl text-sm font-bold ${pos ? "badge badge-emerald" : "badge badge-neg"}`}>
      ...
      <div className="px-5 pb-3">
        ...
        {item.exchange && (
          <div className="mt-2 flex items-center gap-2 text-[10px] text-muted-foreground">
            <span className="rounded-full border border-border px-2 py-0.5">{item.exchange}</span>
  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.05 }} className="relative mb-6 max-w-xl" ref={boxRef}>
        <div className="flex items-center gap-3 glass-input rounded-2xl px-4 py-3">
        ...
        <AnimatePresence>
          {suggestions.length > 0 && (
            <motion.div
              ...
              className="absolute z-20 mt-2 w-full overflow-hidden rounded-2xl border border-border bg-card shadow-2xl"
            >
              {suggestions.map((s, i) => (
                <motion.button
                  key={s.symbol}
                  ...
                  className="flex w-full items-center justify-between px-4 py-2.5 text-left text-sm transition-colors hover:bg-foreground/[0.05]"
                >
