...
import {
  ...
  Eye, Layers, Sparkles, MessageSquare, Newspaper, Compass, Zap,
} from "lucide-react"
            ...
            >
              ...
              <div className="text-right">
                <p className="font-mono text-sm font-semibold tabular-nums text-[var(--text-primary)]">
                  {formatCurrency(item.price, { symbol: item.symbol })}
                </p>
...
function QuickActionsPanel() {
  const actions = [
    { label: "AI Analysis", desc: "Run instant AI evaluation", icon: Sparkles, href: "/markets", highlight: true },
    { label: "Ask Lumora AI", desc: "Chat with financial assistant", icon: MessageSquare, href: "/chat" },
    { label: "Portfolio Planner", desc: "Manage holdings & allocation", icon: Wallet, href: "/portfolio" },
    { label: "Watchlist Desk", desc: "Track equity price movements", icon: Eye, href: "/watchlist" },
    { label: "Market News", desc: "Read breaking financial news", icon: Newspaper, href: "/news" },
    { label: "Macro Signals", desc: "Explore index benchmarks", icon: Compass, href: "/markets" },
  ]
      ...
      <div className="mb-5 flex items-center justify-between">
        <div>
          <p className="meta flex items-center gap-1.5">
            <Zap className="h-3.5 w-3.5 text-[var(--gold)]" /> Quick Actions & Tools
          </p>
          <p className="text-xs text-[var(--text-tertiary)] mt-0.5">Instant tools for market research and portfolio control</p>
        </div>
      </div>
        ...
        {actions.map((act) => {
          ...
          return (
            <Link
              key={act.label}
              href={act.href}
              className={`group flex flex-col justify-between rounded-2xl p-4 transition-all duration-300 border ${
                act.highlight
                  ? "border-[var(--gold)]/30 bg-[var(--gold-glow)] hover:border-[var(--gold)]/60 hover:shadow-[0_0_20px_rgba(212,175,55,0.15)]"
                  : "border-[var(--glass-border)] bg-[var(--glass-bg)] hover:border-[var(--glass-border-hover)] hover:bg-[var(--panel-2)]"
              }`}
            >
              <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-xl bg-[var(--surface)] text-[var(--gold)] shadow-sm">
                <Icon className="h-4 w-4" />
              </div>
              <div>
                <p className="text-xs font-semibold text-[var(--text-primary)] group-hover:text-[var(--gold)] transition-colors">
                  {act.label}
                </p>
                <p className="text-[10px] text-[var(--text-tertiary)] mt-0.5 line-clamp-1">{act.desc}</p>
              </div>
            </Link>
          )
        })}
