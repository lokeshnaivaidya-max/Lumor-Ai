      ...
      <main className="relative z-10 mx-auto max-w-6xl px-6 pb-32 pt-8">
        <div className="lg:grid lg:grid-cols-[280px_1fr] lg:gap-16">
              ...
              <ul className="space-y-1.5">
                {SECTIONS.map((s) => (
                  <li key={s.id}>
                    <a
                      href={`#${s.id}`}
                      className="block rounded-lg px-3 py-1.5 text-sm text-muted-foreground transition-colors hover:bg-foreground/[0.05] hover:text-foreground"
                    >
                      {s.title}
                    </a>
                  </li>
          ...
          <div className="min-w-0">
            <div className="relative overflow-hidden rounded-3xl border border-[var(--line-strong)] bg-[var(--surface)] p-6 sm:p-10 lg:p-12 shadow-xl min-w-0">
              <div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-gold/[0.03] via-transparent to-gold/[0.02]" />
              ...
              <div className="relative">
                ...
                <div className="mt-12 space-y-12 text-sm leading-relaxed text-foreground/80" data-smooth-scroll>
                  ...
                  <Section id="how-we-use-data" title="How We Use Data" icon={Eye}>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      {[
                        ...
                        <div key={item.label} className="rounded-xl border border-[var(--line)] bg-[var(--surface-alt)] px-4 py-3">
                  ...
                  <Section id="cookies" title="Cookies" icon={Cookie}>
                    ...
                    <div className="mt-4 space-y-3">
                      {[
                        ...
                        <div key={c.name} className="flex items-start gap-3 rounded-xl border border-[var(--line)] bg-[var(--surface-alt)] px-4 py-3">
                          <div className="rounded-lg bg-foreground/[0.05] p-1.5 shrink-0 mt-0.5">
                  ...
                  <Section id="third-parties" title="Third Party Services" icon={Shield}>
                    ...
                    {[
                      ...
                      <div key={svc.name} className="mb-3 rounded-xl border border-[var(--line)] bg-[var(--surface-alt)] px-4 py-3">
                        <p className="text-foreground text-xs font-medium">{svc.name}</p>
                        <p className="text-xs text-muted-foreground mt-0.5"><span className="font-medium text-foreground/80">Purpose:</span> {svc.what}</p>
                        <p className="text-xs text-muted-foreground mt-0.5"><span className="font-medium text-foreground/80">Data shared:</span> {svc.data}</p>
                      </div>
                    ))}
                  </Section>
                  ...
                  <Section id="user-rights" title="User Rights" icon={Users}>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      {[
                        ...
                        <div key={item.action} className="flex items-start gap-3 rounded-xl border border-[var(--line)] bg-[var(--surface-alt)] px-4 py-3">
                          <div className="rounded-lg bg-foreground/[0.05] p-1.5 shrink-0 mt-0.5">
                  ...
                  <Section id="children" title="Children" icon={AlertTriangle}>
                    <div className="rounded-xl border border-[var(--line)] bg-[var(--surface-alt)] px-4 py-4">
                      <p>
                  ...
                  <Section id="contact" title="Contact" icon={Mail}>
                    ...
                    <div className="mt-4 flex items-center gap-3 rounded-xl border border-[var(--line)] bg-[var(--surface-alt)] px-4 py-3">
                      <Mail className="h-4 w-4 text-muted-foreground" />
                      <a href="mailto:support@lumora.app" className="text-sm font-medium text-foreground hover:text-gold transition-colors">
                        support@lumora.app
                      </a>
                    </div>
                  </Section>
                ...
                <div className="mt-12 flex items-center gap-4 border-t border-[var(--line)] pt-6">
...
function Section({ id, title, icon: Icon, children }: { id: string; title: string; icon: React.ElementType; children: React.ReactNode }) {
  return (
    <section id={id} className="scroll-mt-24 min-w-0 break-words">
      <div className="flex items-center gap-2.5 mb-4">
        <div className="rounded-lg bg-foreground/[0.05] p-1.5 shrink-0">
