...
function getConfig() {
  ...
  const pass_raw = process.env.SMTP_PASS || process.env.SMTP_PASSWORD || process.env.GMAIL_APP_PASSWORD || process.env.EMAIL_PASS
  const pass = pass_raw ? pass_raw.trim() : undefined
  const from = process.env.SMTP_FROM || process.env.FROM_EMAIL || process.env.EMAIL_FROM || (user ? `"Lumora AI" <${user}>` : undefined)
  ...
  if (!host || !user || !pass || !from) {
    ...
    if (!from) missing.push("SMTP_FROM / FROM_EMAIL")
    return null
  }
...
export const gmailProvider: EmailProvider = {
  ...
  async sendEmail(opts: SendEmailOptions): Promise<SendEmailResult> {
    ...
    const plainText = opts.text || stripHtmlToText(opts.html)

    console.log("[SMTP] Sending mail (multipart/alternative)", JSON.stringify({ to: opts.to, subject: opts.subject }))
    try {
      const info = await transporter!.sendMail({
        ...
        headers: {
          ...
          "X-Entity-Ref-ID": `lumora-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`,
