import { getCurrencyForSymbol, getCurrencySymbol } from "./currency"

export { getCurrencyForSymbol, getCurrencySymbol }
...
export function currencySymbol(currencyOrSymbol: string | undefined): string {
  if (!currencyOrSymbol) return ""
  if (currencyOrSymbol === "$" || currencyOrSymbol === "₹" || currencyOrSymbol === "€" || currencyOrSymbol === "£" || currencyOrSymbol === "¥") {
    return currencyOrSymbol
  }
  const detectedCcy = getCurrencyForSymbol(currencyOrSymbol, "", currencyOrSymbol)
  return getCurrencySymbol(detectedCcy)
}

/**
 * Universal currency formatting utility for Lumora AI.
 * Handles numbers, strings, symbols, exchange names, and currency codes seamlessly.
 */
export function formatCurrency(
  value: number | string | null | undefined,
  currencyOrSymbolOrCode: string | { symbol?: string; exchange?: string; currency?: string } = "USD",
  locale?: string
): string {
  if (value == null || value === "") {
    const sym = typeof currencyOrSymbolOrCode === "string" ? currencySymbol(currencyOrSymbolOrCode) : "₹"
    return `${sym}0.00`
  }
  ...
  // Detect currency code and symbol
  let ccyCode = "USD"
  if (typeof currencyOrSymbolOrCode === "object" && currencyOrSymbolOrCode !== null) {
    ccyCode = getCurrencyForSymbol(
      currencyOrSymbolOrCode.symbol || "",
      currencyOrSymbolOrCode.exchange || "",
      currencyOrSymbolOrCode.currency || ""
    )
  } else if (typeof currencyOrSymbolOrCode === "string") {
    if (currencyOrSymbolOrCode === "₹") ccyCode = "INR"
    else if (currencyOrSymbolOrCode === "$") ccyCode = "USD"
    else if (currencyOrSymbolOrCode === "£") ccyCode = "GBP"
    else if (currencyOrSymbolOrCode === "€") ccyCode = "EUR"
    else if (currencyOrSymbolOrCode === "¥") ccyCode = "JPY"
    else ccyCode = getCurrencyForSymbol(currencyOrSymbolOrCode, "", currencyOrSymbolOrCode)
  }

  const activeSymbol = getCurrencySymbol(ccyCode)
  const activeLocale = locale || (ccyCode === "INR" ? "en-IN" : "en-US")

  const rounded = Math.round((num + Number.EPSILON) * 100) / 100

  let formattedNumber = ""
  try {
    formattedNumber = new Intl.NumberFormat(activeLocale, {
