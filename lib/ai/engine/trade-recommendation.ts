...
export class TradeRecommendationEngine {
  ...
  generateTradePlan(context: TradePlanContext): TradePlan {
    ...
    const institutionalReasoning = {
      ...
      targets: {
        ...
        target2: {
          ...
          supportingFactors: this.buildTargetSupportingFactors(context, suggestedTargets.target2, "Target 2")
        }
      },
      stopLoss: {
    ...
    const tradePlan: TradePlan = {
      ...
      riskScores: {
        ...
        conviction: riskScores.conviction
      },
  ...
  private deriveTargetsFromContext(context: TradePlanContext, currentPrice: number): { target1: number; target2: number } {
    ...
    const conviction = riskScores.conviction
    if (conviction >= 80) {
  ...
  private determineRecommendation(context: TradePlanContext): TradePlan["recommendation"] {
    ...
    if (
      ...
      reasoning.technicals.trendStrength === "strong" &&
      riskScores.trendScore > 60 &&
      riskScores.conviction > 60
    ) {
      return "BUY"
    }
  ...
  private buildEntryJustification(context: TradePlanContext, entry: number, currentPrice: number): string {
    ...
    // Strategy-based entry based on trend and risk
      ...
      const strategy = context.reasoning.technicals.trend === "bullish" ? "Uptrend momentum" : "Bearish trend positioning"
      justification = `${reasonPrefix} ${entry.toFixed(2)} based on ${strategy} with ${context.riskScores.conviction}% conviction.`
    }
  ...
  private buildEntrySupportingFactors(context: TradePlanContext, entry: number, currentPrice: number): string[] {
    ...
    if (reasoning.technicals.trend === "bullish" && reasoning.technicals.trendStrength === "strong") {
    ...
    if (context.quote.volume && context.quote.avgVolume && context.quote.volume > context.quote.avgVolume) {
  ...
  private buildTargetSupportingFactors(context: TradePlanContext, target: number, targetLabel: string): string[] {
    ...
    // Trend and momentum
    if (context.riskScores.conviction > 60) {
      factors.push(`${context.riskScores.conviction}% conviction`)
    }

    // Time-based projections
    const timeHorizon = (context.reasoning as unknown as Record<string, unknown>).horizon as string | undefined || "medium"
    if (timeHorizon === "long") {
  ...
  private buildRecommendationReason(context: TradePlanContext, recommendation: TradePlan["recommendation"]): string {
    ...
    switch (recommendation) {
      case "BUY":
        return `Bullish technical alignment with ${riskScores.conviction}% conviction. Key support levels identified with favorable risk/reward ratio.`
      case "SELL":
        return `Bearish technical alignment with ${riskScores.conviction}% conviction. Key resistance levels present with declining momentum indicators.`
      case "WAIT":
