...
export function AuthForm({ mode, enabledProviders }: { mode: "sign-in" | "sign-up"; enabledProviders: Provider[] }) {
  ...
  const isSignUp = mode === "sign-in" ? false : true
          ...
          <AnimatePresence>
            {resetSuccess && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -4 }}
                className="flex items-start gap-2 rounded-xl border border-emerald/20 bg-emerald/[0.08] px-3.5 py-2.5 text-xs text-emerald"
              >
